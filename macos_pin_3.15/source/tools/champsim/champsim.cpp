...
//#define CAPTURE_PHY_ADDR

#ifdef CAPTURE_PHY_ADDR
#include "veronica.h"
#endif
...
void MemoryRead(VOID* addr, UINT32 index, UINT32 read_size)
{
    ...
    {
        #ifdef CAPTURE_PHY_ADDR
        if(curr_instr.source_memory[i] == (veronica::mem_addr_vir2phy((veronica::uint64)addr)))
        #else
    ...
    if(already_found == 0)
    {
        for(int i=0; i<NUM_INSTR_SOURCES; i++)
        {
            ...
            {
                #ifdef CAPTURE_PHY_ADDR
                curr_instr.source_memory[i] = veronica::mem_addr_vir2phy((veronica::uint64)addr);
                #else
...
void MemoryWrite(VOID* addr, UINT32 index)
{
    ...
    {
        #ifdef CAPTURE_PHY_ADDR
        if(curr_instr.destination_memory[i] == (veronica::mem_addr_vir2phy((veronica::uint64)addr)))
        #else
    ...
    if(already_found == 0)
    {
        for(int i=0; i<NUM_INSTR_DESTINATIONS; i++)
        {
            ...
            {
                #ifdef CAPTURE_PHY_ADDR
                curr_instr.destination_memory[i] = veronica::mem_addr_vir2phy((veronica::uint64)addr);
                #else
