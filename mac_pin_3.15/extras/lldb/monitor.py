...
class MonitorCommand:
    ...
    def __call__(self, debugger, command, exe_ctx, result):
        ...
        if (response):
            # This will return a string of hex values (ascii format)
            stripped = response.group(1)
            
            # Now convert ascii to a string reply
            # str() wrapper is only added for old python 2, not needed for python 3.
            print (str(bytearray.fromhex(stripped).decode()), file=result)
            
            # old way (python 2 only)
